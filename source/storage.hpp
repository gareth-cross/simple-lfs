...
struct ObjectGetter {
 public:
  ...
  virtual std::size_t Read(std::size_t offset, std::size_t length, char* data) = 0;
...
struct Storage {
 public:
  ...
  void PutObject(const lfs::object_t& obj, const std::filesystem::path& upload_path);

  //
  std::shared_ptr<ObjectGetter> GetObject(const lfs::object_t& obj);
 ...
 private:
  ...
  // Pending uploads:
  std::unordered_set<std::shared_ptr<Aws::Transfer::TransferHandle>> pending_transfers_;
